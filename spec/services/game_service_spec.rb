...
RSpec.describe GameService do
  ...
  describe '.get_games' do
    ...
    context 'when the API client returns an error' do
      before { allow(client).to receive(:get_games).and_raise TriviaCrack::Errors::RequestError, 400 }
  ...
  describe '.new_game' do
    ...
    context 'when the API client returns an error' do
      before { allow(client).to receive(:start_new_game).and_raise TriviaCrack::Errors::RequestError, 400 }
