class GamesController < ApplicationController
  ...
  before_action :set_account, only: [:create, :play_game]
  ...
  def index
    ...
    @accounts.each do |account|
      ...
      @queued_games += account.queued_games.map { |game| game.game_id }
    end
  ...
  def new
    ...
    current_user.accounts.each do |account|
      ...
      if user.unlimited_lives || user.lives > 0
        @accounts << account
      end
    end
  end
  ...
  def create
    ...
    respond_to do |format|
      if @game
        ...
        format.html { redirect_to new_game_url, alert: I18n.t('games.unable_to_create_game', username: @account.username) }
